...
def get_repo_versions(repo_id: str) -> list[packaging.version.Version]:
    ...
    try:
        ...
        # If we can't reach HuggingFace, return empty list (for local datasets)
        return []
...
def get_safe_version(repo_id: str, version: str | packaging.version.Version) -> str:
    ...
    # Skip online verification for local datasets
    try:
        hub_versions = get_repo_versions(repo_id)
    except Exception:
        # If we can't reach HuggingFace, assume local dataset and return the target version
        return f"v{target_version}"

    if not hub_versions:
        # For local datasets without HF tags, return the target version directly
        return f"v{target_version}"
