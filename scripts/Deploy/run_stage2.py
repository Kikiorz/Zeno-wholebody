...
# ====== Normalization statistics (from stage2 safetensors) ======
...
def load_policy(ckpt_dir: str, device: str) -> ACTPolicy:
    ...
    rospy.loginfo("=" * 70)
    rospy.loginfo("[INFO] Stage2 Policy loaded successfully")
...
DEFAULT_MODEL_PATH = PROJECT_ROOT / "model" / "stage2_act" / "checkpoints" / "040000" / "pretrained_model"


def main():
    parser = argparse.ArgumentParser(description="Stage2 ACT Policy Deployment (task: move)")
    ...
    # Initialize ROS node
    rospy.init_node("zeno_act_stage2")
    ...
    rospy.loginfo("=" * 70)
    rospy.loginfo("[CONFIG] Stage2 Deployment settings:")
    rospy.loginfo(f"  Task: move")
    ...
    while not rospy.is_shutdown():
        ...
        if step_count % 50 == 1:
            rospy.loginfo("=" * 70)
            rospy.loginfo(f"[Step {step_count}] Diagnostics:")
            rospy.loginfo(f"  Raw state:       {np.array2string(state_raw, precision=3)}")
            rospy.loginfo(f"  Action (raw):    base={np.array2string(action_base, precision=4)}")
            rospy.loginfo(f"  Action (raw):    left={np.array2string(action_left, precision=4)}")
            rospy.loginfo(f"  Action (raw):    right={np.array2string(action_right, precision=4)}")
        ...
        rospy.loginfo_throttle(2.0, f"Stage2 actions sent (base_vx={action_base[0]:.4f})")
